...
// ── CONSULT TIMER HOOK ──
function useConsultTimer(active: boolean) {
...
// ── TODAY ──
function TodayScreen({ openCase, goRx }: { openCase: (id: string) => void; goRx: () => void }) {
  const ME = useClinic((s) => s.currentPractitionerId)
  ...
  const [blockDuration, setBlockDuration] = useState<string | null>(null)
  ...
  function handleScheduleFollowUp(patientId: string, preset: string) {
    ...
    scheduleFollowUp({
      ...
      practitionerId: ME,
      time: '10:00 AM',
      date: dayMap[preset] ?? preset,
      type: 'In person',
      reason: 'Follow-up',
    })
    haptic('success')
    toast({ title: `Follow-up scheduled · ${formatDayLabel(dayMap[preset] ?? preset)}` })
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-2.5">
        {stats.map(([l, v, tone]) => (
          <Card key={l} className="px-3 py-3 text-center">
            <CountUp value={v} className={`font-display text-[22px] font-bold ${tone === 'amber' ? 'text-amber-text' : 'text-ink'}`} />
            <Label className="mt-0.5">{l}</Label>
          </Card>
        ))}
      </div>
      ...
      {activeAppt && activePatient && (
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={springSoft} className="rounded-[22px] p-4 text-white shadow-float" style={{ background: 'linear-gradient(135deg,#5A7C4E,#41603C)' }}>
          ...
          <div className="mt-3.5 flex gap-2">
            <Pressable hap="impact" onClick={() => openCase(activePatient.id)} className="flex items-center gap-1.5 rounded-pill bg-white/95 px-4 py-2 text-[13px] font-semibold text-brand"><NotePencil size={16} weight="fill" /> Case sheet</Pressable>
      ...
      {/* no active consult — quiet state */}
      {!activeAppt && waiting > 0 && (
        <Card className="border-amber/30 bg-amber-tint/30 px-4 py-3">
          <div className="flex items-center gap-2 text-[13px] font-semibold text-amber-text">
            <Clock size={16} weight="fill" /> {waiting} patient{waiting > 1 ? 's' : ''} waiting
          </div>
          <div className="mt-1 text-[12px] text-muted">Tap "Start" on any card below to begin.</div>
        </Card>
      )}

      <div className="flex items-center justify-between">
        <Label>{activeAppt ? 'Up next' : "Today's schedule"}</Label>
        <Pressable hap="tick" onClick={() => { setBlockOpen(true); setBlockDuration(null); setBlockReason('') }} className="text-[12px] font-semibold text-brand">Block time</Pressable>
      </div>
      <motion.div variants={listContainer} initial="hidden" animate="show" className="space-y-2.5">
        {restOfDay.map((a) => {
          const p = pFind(a.patientId)
          if (!p) return null
          const canStart = a.status === 'Waiting' || a.status === 'New' || a.status === 'Upcoming'
          const isSeen = a.status === 'Seen'
          return (
            <motion.div key={a.id} variants={listItem}>
              <Pressable as="div" hap="tick" scale={0.99} onClick={() => openCase(p.id)} className={`flex cursor-pointer items-center gap-3 rounded-[20px] border border-border bg-surface px-3.5 py-3 shadow-card ${isSeen ? 'opacity-60' : ''}`}>
                <div className="text-center">
                ...
                <div className="flex flex-col items-end gap-1.5" onClick={(ev) => ev.stopPropagation()}>
                  ...
                  {canStart && !activeAppt && (
                    <Pressable
                      hap="impact"
                      onClick={() => { startConsult(a.id); haptic('success'); toast({ title: `Consult started · ${p.name}` }) }}
                      className="flex items-center gap-1 rounded-pill bg-brand px-2.5 py-1 text-[11px] font-semibold text-screen"
                    >
      ...
      {/* end consult confirmation */}
      <BottomSheet open={endConsultSheet !== null} onClose={() => setEndConsultSheet(null)}>
        <div className="flex flex-col items-center py-2 text-center">
          ...
          <div className="mt-4 flex w-full gap-2">
            <Pressable hap="tick" onClick={() => setEndConsultSheet(null)} className="flex-1 rounded-pill border border-border bg-surface py-2.5 text-center text-[14px] font-semibold text-body">Cancel</Pressable>
            <Pressable hap="success" onClick={() => endConsultSheet && handleEndConsult(endConsultSheet)} className="flex-1 rounded-pill bg-brand py-2.5 text-center text-[14px] font-semibold text-screen">End &amp; follow-up</Pressable>
          </div>
        </div>
      </BottomSheet>

      {/* schedule follow-up */}
      <BottomSheet open={followUpSheet !== null} onClose={() => setFollowUpSheet(null)}>
        {followUpSheet && (
          <div className="space-y-3">
            <div className="font-display text-[17px] font-bold text-ink">Schedule follow-up</div>
            <div className="text-[12.5px] text-muted">
              For {pFind(followUpSheet)?.name ?? 'patient'}
            </div>
            <div className="space-y-2">
              {['1 week', '2 weeks', '1 month'].map((preset) => (
                <Pressable key={preset} as="div" hap="tick" scale={0.98} onClick={() => handleScheduleFollowUp(followUpSheet, preset)} className="flex cursor-pointer items-center gap-3 rounded-[16px] border border-border bg-surface px-4 py-3">
            ...
            <Pressable hap="tick" onClick={() => setFollowUpSheet(null)} className="w-full rounded-pill border border-border bg-surface py-2.5 text-center text-[14px] font-semibold text-body">Skip for now</Pressable>
          </div>
        )}
      </BottomSheet>

      {/* no-show confirmation */}
      <BottomSheet open={noShowSheet !== null} onClose={() => setNoShowSheet(null)}>
        {noShowSheet && (() => {
          const a = appts.find((ap) => ap.id === noShowSheet)
          ...
          return (
            <div className="flex flex-col items-center py-2 text-center">
              ...
              <div className="mt-4 flex w-full gap-2">
                <Pressable hap="tick" onClick={() => setNoShowSheet(null)} className="flex-1 rounded-pill border border-border bg-surface py-2.5 text-center text-[14px] font-semibold text-body">Cancel</Pressable>
                <Pressable hap="impact" onClick={() => handleNoShow(noShowSheet)} className="flex-1 rounded-pill bg-danger py-2.5 text-center text-[14px] font-semibold text-white">Confirm no-show</Pressable>
              </div>
            </div>
          )
        })()}
      ...
      {/* block time */}
      <BottomSheet open={blockOpen} onClose={() => setBlockOpen(false)}>
        <div className="font-display text-[17px] font-bold text-ink">Block time</div>
        <div className="mt-3">
          <Label>Duration</Label>
          <div className="mt-2 flex gap-2">
            {['30 min', '1 hour', '2 hours'].map((d) => (
              <Chip key={d} selected={blockDuration === d} onClick={() => { haptic('select'); setBlockDuration(d) }} className="flex-1 text-center">{d}</Chip>
            ))}
          </div>
        </div>
        <div className="mt-3">
          ...
          <input
            ...
            placeholder="Lunch, admin, personal..."
        ...
        <Pressable
          ...
          className="mt-4 flex w-full items-center justify-center rounded-pill bg-accent py-3 font-display text-[15px] font-semibold text-white shadow-float"
        >
          Block
        </Pressable>
      </BottomSheet>
